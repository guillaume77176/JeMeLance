...
def get_info_local(base_df, code_insee,code_ape):
    ...
    if concu_df.shape[0] == 0 :
        nb_local_concurrents = 0
    else:
        nb_local_concurrents = concu_df["nb_local_concurrents"].iloc[0]
...
try:
    info_local = get_info_local(base_df, codeInseeCommune,codeAPE)
    revCommune = info_local[0]
    revDep = info_local[1]
    taille_ville = info_local[2]
    nb_local_concurrents = info_local[3]
